...
$lasttime=$con->query('select `time` from komm_news where id_author = '.$user['id'].' order by id desc limit 1')->fetch_assoc();
$lasttime=time()-$lasttime['time'];
$timespam=$con->query('select news from antispam where id=1')->fetch_assoc();
$timespam=$timespam['news'];
...
$con->query("INSERT INTO `komm_news` (`text`, `id_news`, `id_author`, `time`) VALUES ('".$text."', '".$id."', '".$user['id']."', '".time()."')");	
header('Location: ?');
}
}
...
echo '<div class="post">'.user($w['id_author']).'</br>
'.text($w['text']).'</br>'.$ot.'';
